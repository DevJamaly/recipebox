...
/**
 * Base class for all views. Handles the shared render/patch/status-message
 * lifecycle; subclasses only need to implement `_generateMarkup()`.
 */
export default class View {
  ...
  /**
   * Renders data into the view by patching the existing DOM rather than
  ...
  #patch(newParent, oldParent) {
    ...
    for (let i = 0; i < max; i++) {
      ...
      // Both text nodes -> patch the value directly.
      if (
      ...
      // Same tag -> sync attributes (this also fixes the "hidden class" bug), then recurse.
